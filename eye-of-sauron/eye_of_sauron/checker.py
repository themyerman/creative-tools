...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        help="Directory containing YAML rule packs (default: bundled eye_of_sauron/rules).",
    )
    parser.add_argument(
        "--use-semgrep",
        action="store_true",
    ...
    parser.add_argument(
        "--log-dir",
        default="logs",
        help="Directory for per-run scanner logs (relative to current working directory unless absolute).",
    )
    parser.add_argument(
        "--punchlist",
        action="store_true",
        help=(
            "Write markdown + SARIF under TARGET/punchlist/scan-<timestamp>-<id>/ "
            "(punchlist.md + results.sarif) for this run; TARGET is -t/--topdir."
        ),
    )
